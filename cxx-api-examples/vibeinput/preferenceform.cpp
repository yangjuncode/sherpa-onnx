...
PreferenceForm::PreferenceForm(QWidget* parent)
  ...
  // Load denoise method: one of "rnnoise", "gtcrn", "none"; default "none"
  const QString denoise = settings.value(QStringLiteral("denoise_method"), QStringLiteral("none")).toString().toLower();
  int idx = ui->cbx_denoise->findText(denoise, Qt::MatchFixedString);
  if (idx < 0) {
    // fallback to exact items text
    if (denoise == QLatin1String("rnnoise")) idx = ui->cbx_denoise->findText(QStringLiteral("rnnoise"));
    else if (denoise == QLatin1String("gtcrn")) idx = ui->cbx_denoise->findText(QStringLiteral("gtcrn"));
